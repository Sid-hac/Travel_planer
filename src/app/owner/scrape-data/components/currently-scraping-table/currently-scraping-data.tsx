...
export default function ScrapingTable({jobs } : {jobs : jobType[]}) {
  ...
  const renderCell = React.useCallback((job: jobType, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      ...
      case "url" : 
        return <Link href={cellValue} className="text-primary-500" target="_blank">{cellValue}</Link>;
      case "createdAt" :
        return formatDateTime(cellValue);
      case "status" :
        return (
          <Chip className="capitalize " color={statusColorMap[job.status]} size="sm" variant="flat">
            {cellValue}
          </Chip>
        );
      default:
